...
description: "アーキテクチャレビューのための知識構築エージェント（Phase 1-3）。システムマップ収集、不変条件抽出、コンポーネントカード作成を順次実行し、RAGベースの分析基盤を構築する。使用タイミング: (1) 「アーキテクチャの知識を構築して」、(2) 「システムマップを作成して」、(3) 「コンポーネントカードを作成して」、(4) アーキテクチャレビューの準備段階"

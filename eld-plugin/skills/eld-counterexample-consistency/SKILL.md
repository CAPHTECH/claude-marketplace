...
description: 要件・形式仕様・テスト・コード・実行時観測を要件IDで結び、反例・変異・契約違反・トレース違反で多層整合性を検査する。Use when 要件からコードまでの整合性を確認したい、トレーサビリティを設計したい、反例駆動で仕様差分を検出したい、「反例駆動でチェックして」「多層整合性を見て」と言われた時。
---

...
name: eld
context: fork
description: Evidence-Loop Development (ELD) v3 - 証拠で回す統合開発手法。5+1フェーズ（Sense→Spec→Change→Ground→Record+Predict-Lightゲート）でコード観測・Spec規範・安全な変更・知識管理を統一ループで実行する。「ELDで進めて」「証拠ループで実装して」「コードベースを理解して」「影響範囲を分析して」「Specを定義して」「Termをカード化して」「安全に変更して」「証拠パックを作成して」「コンテキスト駆動で実装して」「ELDでデバッグして」「法則視点でバグ調査して」等で使用。新機能開発、バグ修正、リファクタリング、障害調査に対応。
---

# Evidence-Loop Development (ELD) v3
...
| Phase | 内容 | 参照 |
|-------|------|------|
| **Sense** | コードの事実/意図/関係を観測、身体図を更新 | references/10-sense.md |
| **Spec** | 語彙（Term）と関係（Law）を同定、カード化、Link Map管理 | references/20-spec.md |
| **Predict-Light** (gate) | リスクを P0/P1/P2 で軽量判定、High時のみ詳細分析 | references/30-predict.md |
| **Change** | 最小単位で変更、Pure/IO分離を優先、検証通過後コミット | references/40-change.md |
| **Ground** | テスト/Telemetry/再現手順で接地、Review Hybrid | references/50-ground.md |
| **Record** | Context DeltaをADR/Catalog/tasks/lessons.mdへ反映 | references/60-record.md |
...
詳細は references/00-glossary.md を参照。
...
1. 目的に応じたツールでコード調査（references/10-sense.md 参照）
   - 特定キーワード → `Grep`
   - 定義・参照追跡 → `LSP` / `serena`
   - シンボル検索 → `serena`（利用可能時）
   - 意味的関連探索 → `kiri context_bundle`
3. Issue Contractを作成（references/issue-template.md使用）
4. `/eld-spec` で Term/Law候補を列挙

使用スキル: `/eld-sense-requirements-brainstorming` (新機能時), `/eld-sense-planning`, `/eld-spec`
...
使用スキル: `/eld-predict-light`, `/eld-change-worktree`, `/eld-ground`, `/eld-record`
...
PR作成: references/pr-template.md 使用
...
| ストア | 役割 | 内容 |
|--------|------|------|
| tasks/lessons.md | 学びの記録 | パターン、意思決定、エラー解決策 |
...
### 概念定義
- references/00-glossary.md - 統一語彙

### フェーズ詳細
- references/10-sense.md - 感知フェーズ
- references/20-spec.md - Specフェーズ（Law/Term発見・カード化）
- references/30-predict.md - Predict-Lightフェーズ（軽量リスク判定）
- references/40-change.md - 変更フェーズ（コミット含む）
- references/50-ground.md - 接地フェーズ
- references/60-record.md - 記録フェーズ

### テンプレート
- references/issue-template.md - Issue Contract雛形
- references/pr-template.md - PR & Evidence Pack雛形
- references/law-term-card.md - Law/Term Card雛形
- references/predict-light-template.md - Predict-Light判定雛形
- references/claim-schema.md - Claim Schema定義
- references/review-hybrid-guide.md - Review Hybrid実施ガイド
...
| スキル | 用途 |
|--------|------|
| `/eld-sense-planning` | アクティブコンテキスト構築・タスクスコープ定義・タスク分解・並列実行最適化 |
...
| `/eld-ground` | 接地検証・Review Hybrid（Artifact-Based + Line Review）・TDD統制・PR作成前検証 |

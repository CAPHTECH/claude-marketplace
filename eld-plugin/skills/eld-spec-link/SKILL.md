...
name: eld-spec-link
context: fork
description: ELD SpecのLink Map（連結表）管理スキル。Law↔Term の相互参照関係を可視化し、孤立の検出と影響分析を行う。使用タイミング: Law Cardを作成した後、Term Cardを作成した後、「Link Mapを更新して」「孤立チェックして」、変更影響分析時。
---
...
### Law Card作成後（`/eld-spec-card law` の後）
...
### Term Card作成後（`/eld-spec-card term` の後）

...
| スタイル | 用途 | 実装 | 特徴 |
|---------|------|------|------|
| 階層型（Hierarchical） | コンテンツの閲覧、設定画面 | `NavigationStack` with push | 明確な親子関係、戻りパスが一意 |
| フラット型（Flat） | アプリのメインナビゲーション | `TabView` | どのタブからも他タブへ即座にアクセス可能 |
| コンテンツ駆動型（Content-driven） | ページング、ブック、ゲーム | カスタムナビゲーション | コンテンツの論理的順序に従う |

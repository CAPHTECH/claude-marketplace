---
name: zellij-issue-room
description: Issue番号を指定してworktreeとzellij開発環境を構築する。「Issueに取り組んで」「Issue #45の作業環境を作って」と言われた時に使用する。
...
Issue専用の開発環境をzellij上に構築する。

## 概要

```
<issue-number> → gh issue view → ブランチ名生成 → worktree作成 → zellij tab構築
```
...
1. gitリポジトリ内か
2. `gh` CLIが利用可能か
3. `zellij` セッション内か（`$ZELLIJ` 変数の存在）— 非zellij時はworktree作成とパス出力のみ
...
作業完了後は `zellij-worktree delete <branch>` と同等の操作で環境を削除する。

## エラーハンドリング

- Issue番号が無効: `gh issue view` のエラーメッセージを表示
- ブランチ名衝突: 既存worktreeが見つかった場合はそれを使用
- zellij非実行時: worktree作成とIssue情報表示のみ行う

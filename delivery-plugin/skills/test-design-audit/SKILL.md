...
context: fork
description: 要求トレーサビリティ（REQ→TCND→TEST）とカバレッジモデルによりテスト設計の抜け漏れを防止する監査スキル。モデル化・複数視点監査で体系的に検証する。Use when: テスト設計、テスト計画作成、QA開始前、「テスト漏れがないか不安」と感じた時
---
...
- 機能（Functional）: 機能要件、主要フロー
- 非機能（Non-functional）: 性能・可用性などの品質特性
- 制約（Constraint）: 入力形式・桁数などの境界条件
- 運用（Operational）: 障害対応・監視・リリース手順
- 法令・規約（Regulatory）: コンプライアンス要件
- UX（User Experience）: ユーザー体験に関する要求
...
**制約**: 不正な状態遷移（「退会済み→ログイン中」は不可など）を明示する。
...
**契約**: API契約を事前条件（入力制約）/事後条件（出力保証）として明示する。
...
| TEST-003 | TCND-007 | REQ-004 | 外部API停止中 | 任意 | フォールバック動作 |
```
...
- 関連（任意）: Law/Term起点で接地状況を検証したい場合は `/eld-ground verify` も参照可能

...
| `issue-discoverer` | コード走査によるIssue候補の自律発見・承認・起票 |
...
| `main-merge-resolver` | 現在ブランチへ既定でmainを取り込み、必要ならコンフリクト解消 |
...
| `coderabbit-config` | CodeRabbit設定ファイルの生成・最適化 |
...
| スキル | 説明 |
|--------|------|
| `op-env` | 1Password CLI による `.env` シークレット管理 |
...
- **eld-plugin** との併用を推奨。`issue-workflow-orchestrator` が `/eld`, `/eld-sense-planning`, `/eld-ground review` を参照します。`test-design-audit` が Law/Term概念を参照します。

## CLAUDE.md セットアップ

一部のスキルは、プロジェクトの `CLAUDE.md` に指示を追記することで効果的に機能します。

### プロアクティブIssue発見（issue-discoverer）

以下をプロジェクトの `CLAUDE.md` に追記すると、AIが作業中に発見した課題を積極的にIssue候補として報告します:

```markdown
## Proactive Issue Discovery
作業中にスコープ外のバグ・セキュリティ脆弱性・重要なテスト不足を発見した場合は、
/issue-discoverer で候補化し、ユーザーに承認を求めること。
現タスクに直接関係する問題は現タスク内で修正する。
```

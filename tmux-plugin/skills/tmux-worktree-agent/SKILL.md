...
**全エージェント共通で、指示の冒頭に以下を必ず含める:**

```
/task-plan-builder, /agent-coding-preflight, /uncertainty-resolution を活用して取り組むこと。
```

**エージェント種別ごとに、上記の直後に以下も必ず含める:**
- **claude の場合**: 「TeamCreateでチームを作成し、複数のsubagentで並列に作業を進めること。」
- **codex の場合**: 「Agentツールでsubagentを積極的に活用し、調査・実装・検証を並列化すること。」
